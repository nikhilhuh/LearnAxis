...
import { getTeacherOngoingBatches } from "../../services/api/apiCalls/teacher/getTeacherOngoingBatches";
...
const TeacherAttendance: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchBatches = async () => {
      ...
      setLoadingData(true);
      try {
        const result = await getTeacherOngoingBatches(UserDetails.email);
              ...
              <div className="grid grid-cols-1 tablet:grid-cols-2 laptop-sm:grid-cols-3 gap-4 tablet:gap-8">
                {batches.map((batch, idx) => (
                  <BatchData
                    ...
                    onClick={() =>
                      navigate(
                        `/teacher/attendance/${encodeURIComponent(batch.name)}`,
                        {
