...
import { getStudentBatches } from "../../services/api/apiCalls/student/getStudentBatches";
import { useNavigate } from "react-router-dom";

const StudentBatches: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchBatches = async () => {
      ...
      setLoadingData(true);
      try {
        const result = await getStudentBatches(UserDetails.email);
              ...
              <div className="grid grid-cols-1 tablet:grid-cols-2 laptop-sm:grid-cols-3 gap-4 tablet:gap-8">
                {batches.map((batch, idx) => (
                  <BatchData
                    ...
                    batch={batch}
                    onClick={() =>
                      navigate(`/student/batches/${encodeURIComponent(batch.name)}`, {
